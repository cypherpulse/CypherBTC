...
describe('cypher-btc-token', () => {
  it('should set the correct contract owner on deploy', () => {
    const ownerCall = simnet.callReadOnlyFn('cypher-btc-token', 'get-contract-owner', [], deployer);
  ...
  it('mint called by contract owner succeeds and updates balances and total supply', () => {
    ...
    const mintCall = simnet.callPublicFn('cypher-btc-token', 'mint', [Cl.uint(amount), Cl.principal(wallet1)], deployer);
    expect(mintCall.result).toBeOk(Cl.bool(true));

    // Check recipient balance
    const balanceCall = simnet.callReadOnlyFn('cypher-btc-token', 'get-balance', [Cl.principal(wallet1)], wallet1);
    expect(balanceCall.result).toBeOk(Cl.uint(amount));

    // Check total supply
    const totalSupplyCall = simnet.callReadOnlyFn('cypher-btc-token', 'get-total-supply', [], deployer);
    expect(totalSupplyCall.result).toBeOk(Cl.uint(amount));
  });

  it('mint called by non-owner fails', () => {
    const amount = 1000000;
    const mintCall = simnet.callPublicFn('cypher-btc-token', 'mint', [Cl.uint(amount), Cl.principal(wallet1)], wallet1);
  ...
  it('transfer succeeds when sender has sufficient balance', () => {
    ...
    // Mint to wallet1
    simnet.callPublicFn('cypher-btc-token', 'mint', [Cl.uint(mintAmount), Cl.principal(wallet1)], deployer);

    // Transfer from wallet1 to wallet2
    const transferCall = simnet.callPublicFn('cypher-btc-token', 'transfer', [Cl.uint(transferAmount), Cl.principal(wallet1), Cl.principal(wallet2)], wallet1);
    expect(transferCall.result).toBeOk(Cl.bool(true));

    // Check balances
    const balance1Call = simnet.callReadOnlyFn('cypher-btc-token', 'get-balance', [Cl.principal(wallet1)], wallet1);
    expect(balance1Call.result).toBeOk(Cl.uint(mintAmount - transferAmount));

    const balance2Call = simnet.callReadOnlyFn('cypher-btc-token', 'get-balance', [Cl.principal(wallet2)], wallet2);
  ...
  it('get-name returns CypherBTC', () => {
    const nameCall = simnet.callReadOnlyFn('cypher-btc-token', 'get-name', [], deployer);
    expect(nameCall.result).toBeOk(Cl.stringAscii('CypherBTC'));
  });

  it('get-symbol returns cBTC', () => {
    const symbolCall = simnet.callReadOnlyFn('cypher-btc-token', 'get-symbol', [], deployer);
    expect(symbolCall.result).toBeOk(Cl.stringAscii('cBTC'));
  });

  it('get-decimals returns 6', () => {
    const decimalsCall = simnet.callReadOnlyFn('cypher-btc-token', 'get-decimals', [], deployer);
